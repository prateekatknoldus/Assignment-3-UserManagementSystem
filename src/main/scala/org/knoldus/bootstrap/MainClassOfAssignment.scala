...
object MainClassOfAssignment {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val userDB:Dao[User] = new UserDatabase() // instance of UserRepository is created
    ...
    val user1 = User(None, "prateek", "12345", "prateek123@gmail.com", Type.Customer) // instance of User class is created
    val user1_id = userService.createUser(user1) // creates user1 and returns its ID

    // creating user2 which is of type Admin
    val user2 = User(None, "chaitanya", "54321", "shubham546@gmail.com", Type.Admin) // instance of User class is created
    val user2_id = userService.createUser(user2) // creates user2 and returns its ID

    // Step 2 => Listing or Getting all users
    println(userService.getAllUsers) // prints the list of all users after retrieving it from the database.
    ...
    println(userService.getUserByID(user2_id)) // prints the updated Admin(i.e. user2) details after retrieving it from the database.
